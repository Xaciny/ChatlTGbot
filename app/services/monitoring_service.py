...
class MonitoringService:
    ...
    async def get_admin_ids(self) -> list[int]:
        try:
            ...
            admin_ids = [
                admin.user.id for admin in chat_admins if not admin.user.is_bot
            ]
            return admin_ids
    ...
    async def check_and_notify(self):
        ...
        logger.info(
            f"Проверка баланса: {balance:.2f} {currency}, расход в день: {daily_cost:.2f} {currency}, осталось дней: {days_remaining}"
        )

        if days_remaining < 1:
            await self._send_low_balance_alert(
                balance, currency, days_remaining, daily_cost
            )
    ...
    async def _send_low_balance_alert(
        self, balance: float, currency: str, days: int, daily_cost: float
    ):
        message = (
        ...
        for admin_id in admin_ids:
            try:
                ...
                logger.info(
                    f"Отправлено критическое уведомление администратору {admin_id}"
                )
            except Exception as e:
