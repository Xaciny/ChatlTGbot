...
from app.services import MediaService, MessageService
...
@group_router.message(
    lambda message: message.chat.id == settings.GROUP_ID and message.reply_to_message
)
async def handle_reply(message: Message, bot: Bot):
    ...
    original_mapping = await MessageService.get_mapping_by_group(
    ...
    if message.text:
        sent_message = await bot.send_message(
            ...
            reply_to_message_id=original_mapping["user_message_id"]
            if original_mapping
            else None,
        )
        ...
        if media:
            caption = (
                f"Материалы от редакции:\n\n{message.caption}"
                if message.caption
                else "Материалы от редакции"
            )
            sent_message = await media_service.send_media(
                original_user_id, media_type, media.file_id, caption
            )

    if sent_message:
        await MessageService.save_mapping(
            message.message_id, original_user_id, sent_message.message_id
        )
        await MessageService.set_last_reply(original_user_id, message.message_id)
        logger.info(
            f"Сохранён маппинг для ответа: {message.message_id} -> {original_user_id}:{sent_message.message_id}"
        )
    else:
...
async def handle_edited_message(message: Message, bot: Bot):
    logger.info(
        f"Обработчик handle_edited_message вызван. ID сообщения: {message.message_id}"
    )
    ...
    now = datetime.now()
    message_date = (
        message.date.replace(tzinfo=None) if message.date.tzinfo else message.date
    )
