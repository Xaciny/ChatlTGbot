...
from app.services import MediaService, MessageService, UserService
...
async def forward_to_group(message: Message, bot: Bot):
    ...
    if await UserService.is_banned(message.from_user.id):
        await message.reply(
            "Вы заблокированы администратором. Обратитесь в редакцию для разрешения ситуации."
        )
        return
    ...
    await MessageService.save_mapping(
        sent_message.message_id, message.from_user.id, message.message_id
    )
...
@private_router.message(
    lambda message: (
        message.chat.type == "private"
        and (message.photo or message.video or message.document or message.animation)
    )
)
