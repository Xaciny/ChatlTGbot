...
def load_welcome_message() -> str:
    if settings.WELCOME_FILE.exists():
        try:
            with open(settings.WELCOME_FILE, encoding="utf-8") as f:
...
def extract_user_id(reply_message: Message) -> int:
    try:
        if reply_message.text:
            id_str = (
                reply_message.text.split("ID пользователя: ")[1]
                .split("\n")[0]
                .replace("#", "")
            )
        elif reply_message.caption:
            id_str = (
                reply_message.caption.split("ID пользователя: ")[1]
                .split("\n")[0]
                .replace("#", "")
            )
        else:
            raise ValueError(
                "Сообщение не содержит текста или подписи с ID пользователя"
            )
        ...
        raise ValueError("Не удалось извлечь ID пользователя") from e
